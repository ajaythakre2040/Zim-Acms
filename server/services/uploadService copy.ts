...
const saveToFile = (folder: string, filename: string, data: any) => {
    fs.writeFileSync(path.join(folder, filename), Papa.unparse(data));
};

// --- Employee Bulk Update Service (Strictly Update Existing Only) ---
export const processEmployeeBulkUpdateOnly = async (data: any[]) => {
    const errors: any[] = [];
    const ts = Date.now();
    saveToFile(DIRS.DETAILS_UPLOADS, `upload_${ts}.csv`, data);

    // Load Lookup Maps for Name-to-ID mapping
    ...
    const desigMap = new Map(desigList.map(d => [d.name.toLowerCase(), d.id]));
    ...
    for (const row of data) {
        ...
        try {
            const departmentId = depMap.get(row.department_name?.toLowerCase());
            const designationId = desigMap.get(row.designation_name?.toLowerCase());

            // Perform Update (Strictly by employeeCode)
            ...
            if (result.length === 0) {
                errors.push({ ...row, error: "Employee Code not found in database" });
                continue;
            }

            // Sync with MS SQL using EmployeeCode
            const updated = result[0];
            if (updated && updated.employeeCode) {
                try {
                    await dbMsSql
                        .update({ dbName: "Employees", pk: "EmployeeCode" })
    ...
    if (errors.length > 0) {
        const fileName = `errors_${ts}.csv`;
        saveToFile(DIRS.DETAILS_ERRORS, fileName, errors);
...
export const processDoorUpdate = async (data: any[]) => {
    const errors: any[] = [];
    const ts = Date.now();
    saveToFile(DIRS.DOORS_UPLOADS, `upload_${ts}.csv`, data);

    // 1. Doors ka Lookup Map load karein (Name -> ID)
    const doorsList = await db.select().from(doors); // 'doors' table hona chahiye
    const doorMap = new Map(doorsList.map(d => [d.name.toLowerCase(), d.id]));

    for (const row of data) {
        try {
            // 2. Door Names ko split karke IDs mein convert karein
            // CSV format: "Main Gate, Office Door, Server Room"
    ...
    if (errors.length > 0) {
        const fileName = `errors_${ts}.csv`;
        saveToFile(DIRS.DOORS_ERRORS, fileName, errors);
