...
import { Pencil, Plus, ChevronsRight, ChevronRight, ChevronLeft, ChevronsLeft } from "lucide-react";
...
export default function VisitorsPage() {
  ...
  const fetchVisitors = async () => {
    try {
      ...
      console.error("Visitors registry fetch failed:", err);
    }
  };
  ...
  const createVisitor = useMutation({
    ...
    onError: (e: Error) => toast({ title: "Registration Error", description: e.message, variant: "destructive" }),
  });
  ...
  const updateVisitor = useMutation({
    ...
    onError: (e: Error) => toast({ title: "Update Error", description: e.message, variant: "destructive" }),
  });

  // 3. Schema Fields Layout for Modal Dialog Form
  const visitorFields: FieldConfig[] = [
    ...
    { key: "rfidCardNo", label: "RFID Card No" },
    { key: "whomToMeet", label: "Whom To Meet (ZIM Employee)", required: true },
  ...
  const columns = [
    { key: "nameOfVisitor", label: "Name", render: (v: any) => <span className="font-semibold text-foreground">{v.nameOfVisitor}</span> },
    ...
    { key: "rfidCardNo", label: "RFID Card", render: (v: any) => v.rfidCardNo || "-" },
    {
      key: "validity",
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
      <PageHeader title="Visitor Registry" description="Manage visitor data, company profiles, and custom permissions" />

      {/* Top Controls Action Toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Button onClick={() => { setEditingVisitor(null); setVisitorDialog(true); }}>
        ...
        <div className="relative max-w-sm w-full">
          <input
            ...
            onChange={(e) => {
              ...
              setPage(1); // Reset back to first page on keystroke
            }}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-4 py-4 border-t bg-muted/20 rounded-b-lg">
        <div className="text-sm text-muted-foreground order-2 md:order-1">
          ...
          <span className="font-semibold text-foreground">{totalCount}</span> registered visitors
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 md:gap-8 order-1 md:order-2">
          ...
          <div className="flex items-center space-x-1">
            ...
            <Button variant="outline" size="sm" className="h-8 px-3 text-xs font-medium gap-1" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}>
            ...
            <Button variant="outline" size="sm" className="h-8 px-3 text-xs font-medium gap-1" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages}>
