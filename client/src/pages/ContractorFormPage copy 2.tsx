...
export default function ContractorFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...restFormData,
      associatedWithZimInYears: formData.associatedWithZimInYears
        ? parseInt(formData.associatedWithZimInYears)
        : null,
      manpowerServiceAvailableInZim: formData.manpowerServiceAvailableInZim
        ? parseInt(formData.manpowerServiceAvailableInZim)
        : null,
      totalManpowerServiceAvailableWithTheVendor:
        formData.totalManpowerServiceAvailableWithTheVendor
          ? parseInt(formData.totalManpowerServiceAvailableWithTheVendor)
          : null,
      licensedQuantity: formData.licensedQuantity
        ? parseInt(formData.licensedQuantity)
        : null,
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="w-full border rounded p-2 text-sm bg-slate-50 disabled:opacity-60"
          />
        </div>
        <div>
          <label className="block text-xs font-bold mb-1 text-slate-700">
...
<div>
  ...
  {/* Parent grid remains 3 columns for perfect bottom alignment */}
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
    
    {/* Row 1: Dono address line ko md:col-span-3 dekar full width kiya, fir andar half-half divide kiya */}
    ...
    {/* Row 2: Ye teeno automatic 3-column layout me barabar fit ho jayenge */}
    <div>
      <label className="block text-xs font-bold mb-1 text-slate-700">
        District
      </label>
...
{activeTab === "operations" && (
  ...
  {/* Left: Optional debug or status text */}
  ...
  <div className="flex items-center gap-3">
    {/* Previous Button - Only show if we aren't on the first tab */}
    {activeTab !== "personal" && (
      <Button
        type="button"
        variant="outline"
        onClick={() => {
          console.log("Previous Clicked", activeTab);
    ...
    {activeTab !== "operations" ? (
      <Button
        type="button"
        onClick={(e) => {
          e.preventDefault(); // Prevents accidental form submission on step change
          console.log("Next Clicked", activeTab);
