...
import {
  ...
  exportAuditTrailCSV,
  exportAuditTrailPDF,
  exportLoginSessionCSV,
  exportLoginSessionPDF,
  exportCSV,
  formatDateTime,
} from "@/lib/utils";
...
export default function AuditTrailPage() {
  const { canView } = usePermission(
    MENU_CONFIG.AUDIT_TRAIL?.code || "audit_01",
  );

  const { canView: canExport } = usePermission(
    MENU_CONFIG.AUDIT_TRAIL?.code || "audit_01",
  );
  if (!canView) {
    return (
  ...
  const handleExport = async (tabType: "activity" | "sessions") => {
    try {
      ...
      exportCSV(
        tabType === "activity" ? "Audit_Activity_Logs" : "Login_Sessions_Logs",
        mappedData,
      );
    } catch (error) {
  ...
  const exportFileName =
    activeTab === "activity" ? "Audit_Activity_Logs" : "Login_Sessions_Logs";
  ...
  const auditColumns = [
    ...
    },
    {
      ...
      render: (item: any) => {
        ...
        if (action === "EDIT" || action === "UPDATE") {
          return (
            <Badge className="bg-amber-100 text-amber-700 border-amber-300 hover:bg-amber-200 font-bold tracking-wide shadow-sm">
              UPDATE
            </Badge>
          );
        }
        if (action === "ADD/EDIT" || action === "CREATE/UPDATE") {
          return (
            <Badge className="bg-sky-100 text-sky-700 border-sky-300 hover:bg-sky-200 font-bold tracking-wide shadow-sm">
              ADD/UPDATE
            </Badge>
          );
        }
        if (action === "INSERT" || action === "CREATE") {
          return (
            <Badge className="bg-emerald-100 text-emerald-700 border-emerald-300 hover:bg-emerald-200 font-bold tracking-wide shadow-sm">
              CREATE
            </Badge>
          );
        }
        if (action === "DELETE") {
          return (
            <Badge className="bg-rose-100 text-rose-700 border-rose-300 hover:bg-rose-200 font-bold tracking-wide shadow-sm">
              DELETE
            </Badge>
          );
        }
    ...
    },
    {
      ...
      label: "Performed By",
      render: (item: any) => (
        <span className="text-violet-700 font-semibold bg-violet-50 px-2 py-0.5 rounded-full text-xs border border-violet-100">
  ...
  const loginColumns = [
    ...
    },
    {
      ...
      label: "IP Address",
      render: (item: any) => (
        <span className="text-cyan-700 font-mono text-sm bg-cyan-50 px-2 py-0.5 rounded border border-cyan-100">
          {item.ipAddress || "-"}
        </span>
      ),
    },
    {
      ...
      render: (item: any) => {
        ...
        if (status === "LOGIN" || status === "ACTIVE")
          return (
            <Badge className="bg-emerald-100 text-emerald-700 border-emerald-300 hover:bg-emerald-200 font-bold">
              LOGGED IN
            </Badge>
          );
        if (status === "LOGOUT")
          return (
            <Badge className="bg-slate-100 text-slate-600 border-slate-300 hover:bg-slate-200 font-bold">
              LOGGED OUT
            </Badge>
          );
        return (
      ...
      render: (item: any) => (
        <span className="text-sm font-medium">
          {item.logoutAt ? (
            ...
            <span className="text-emerald-600 font-bold text-xs bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full animate-pulse">
              ACTIVE SESSION
            </span>
          )}
        </span>
    ...
    <div className="p-4 md:p-6 bg-slate-50/50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 font-medium">
          Monitor system activity and session logs
        </p>
      </div>
      <Tabs
        value={activeTab}
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <TabsList className="grid w-full max-w-md grid-cols-2 bg-slate-100 p-1 rounded-lg">
            <TabsTrigger
              value="activity"
              className="data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm font-semibold transition-all"
            >
              Activity Logs
            </TabsTrigger>
            <TabsTrigger
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-8 order-1 md:order-2">
              ...
              <div className="flex items-center space-x-1">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 px-3 text-xs font-bold gap-1 text-slate-600 border-slate-200 shadow-sm bg-white"
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-4 py-4 border-t border-slate-100 bg-slate-50/50 mt-4 rounded-b-lg">
            <div className="text-sm text-slate-500 order-2 md:order-1 font-medium">
              ...
              <span className="font-bold text-slate-800">
                {Math.min(
                  loginPage * loginPageSize,
                  loginResponse?.totalCount || 0,
                )}
              </span>{" "}
              of{" "}
              <span className="font-bold text-slate-800">
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-8 order-1 md:order-2">
              ...
              <div className="flex items-center space-x-1">
                ...
                <Button
                  ...
                  size="icon"
                  className="h-8 w-8 text-slate-500 hover:text-slate-800"
                  onClick={() => setLoginPage(loginTotalPages)}
                  disabled={loginPage === loginTotalPages}
                >
                  <ChevronsRight className="h-4 w-4" />
                </Button>
          ...
          {selectedLog && (
            <div className="space-y-4 mt-2">
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 text-sm font-medium">
                <div>
                  <span className="text-slate-400 block text-xs font-bold uppercase tracking-wide">
                    Module / Table
                  </span>
